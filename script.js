(() => {
  ...
  // Side rails: ruler slides, progress fills, marker rotates with scroll
  ...
  // Floating architectural shapes drift with the scroll
  ...
  // Current section number in the left rail
    ...
    const so = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          ...
          // rails switch to light type over the dark sections
...
// Hero background video: the smallest sensible file, loaded only when it makes sense
(() => {
  ...
  // poster only: saves the whole download on slow links, weak devices and reduced motion
  if (slowNetwork || weakDevice || reduceMotion) return;

  // pick the variant that matches the actual pixel width of the hero
  ...
  // start after the page has settled, so first paint stays fast
