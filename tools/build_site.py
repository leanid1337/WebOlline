...
SITE_URL = "https://www.olline.design"          # <-- поменяйте на свой домен, без слэша в конце
...
def main():
    ...
    for name in TEXT_FILES:
        ...
        text = text.replace("https://www.olline.design", SITE_URL)
    ...
    for name in COPY_FILES:
        ...
        if name.endswith((".ico", ".png", ".jpg", ".webp")):   # двоичные копируем как есть
            ...
            continue
        text = open(src, encoding="utf-8").read().replace("https://www.olline.design", SITE_URL)
        open(os.path.join(DIST, name), "w", encoding="utf-8").write(text)
